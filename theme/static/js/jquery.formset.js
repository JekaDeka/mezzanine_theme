    ...
    $.fn.formset = function(opts) {
        var options = $.extend({}, $.fn.formset.defaults, opts),
            ...
            updateElementIndex = function(elem, prefix, ndx) {
                ...
                if (elem.is("div") && !elem.attr("class")) {
                    var a_add = $(elem).find('a.fb_show');
                    var a_clear = $(elem).find('p.mezz-fb-clear > a');
                    if ($(a_add).attr('href')) $(a_add).attr('href', $(a_add).attr("href").replace(idRegex, replacement));
                    if ($(a_clear).attr('href')) $(a_clear).attr('href', $(a_clear).attr("href").replace(idRegex, replacement));
                    if ($(elem).find('p.mezz-fb-thumbnail').attr("id")) $(elem).find('p.mezz-fb-thumbnail').attr("id", $(elem).find('p.mezz-fb-thumbnail').attr("id").replace(idRegex, replacement));
                    if ($(elem).find('p.mezz-fb-clear').attr("id")) $(elem).find('p.mezz-fb-clear').attr("id", $(elem).find('p.mezz-fb-clear').attr("id").replace(idRegex, replacement));
                    updateElementIndex($(elem).find('p > a'), prefix, ndx);
                    updateElementIndex($(elem).find('p > a > img'), prefix, ndx);
                }
